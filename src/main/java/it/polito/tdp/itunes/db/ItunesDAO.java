...
public class ItunesDAO {
	...
	public List<Track> getAllTracks(Map<Integer,Track> idMap){
	...
	public List<Track> getCanzoniGenere(Genre genere,Map<Integer,Track> idMap){
		final String sql="SELECT Trackid "
				+ "FROM track "
				+ "WHERE Genreid=? ";
		...
		try {
			...
			while(rs.next()){
			result.add(idMap.get(rs.getInt("trackid")));	
			}
			conn.close();
		}catch (SQLException e) {
			e.printStackTrace();
	...
	public List<Arco> getArchi(Genre genere,Map<Integer,Track> idMap){
		final String sql="SELECT t1.TrackId,t2.TrackId,abs(t1.Milliseconds-t2.Milliseconds) AS dif "
		...
		try {
			...
			ResultSet rs= st.executeQuery();
			while (rs.next()) {
				Arco a = new Arco(idMap.get(rs.getInt("t1.TrackId")),idMap.get(rs.getInt("t2.TrackId")),rs.getInt("dif"));
				result.add(a);
			}
			conn.close();
		} catch (SQLException e) {
